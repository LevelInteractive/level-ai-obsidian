...
description: >
  ...
  each file's k-nearest neighbors (wiki-first, then raw data if no confident wiki fit), has Claude
...
is similar to this file?" for every new or stale file, lets Claude judge which candidates are
...
`ob-wiki-update` is the separate bounded evidence-review and wiki-writing step; this pair provides
...
- **Graph state**: `.claude\similarity-graph.json` — shared with `ob-similarity-consolidate`; this
...
Read `.claude\similarity-graph.json`.
...
2. **Write a 2-3 sentence summary** capturing its topic, entities involved, and key facts. This
   ...
   than incidental phrasing. **For files under `Data\Claude\`**, summarize what was actively worked
...
6. Query qmd across both collections:
   ...
   about generating a broad candidate set for Claude to judge, not finding the single best answer.
   A tighter default would silently exclude real-but-weaker connections before Claude ever sees them.
...
1. Write the updated graph back to `.claude\similarity-graph.json`, with `lastRun` set to today.
...
- **qmd supplies candidates; Claude decides membership.** Never confirm a neighbor, or resolve a
