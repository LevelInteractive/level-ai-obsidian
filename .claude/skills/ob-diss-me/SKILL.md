...
- Commitments assigned to the user in meeting notes or wiki pages that show no closure
...
- Commitments made ("I'll have that by EOD", "I'll take care of it") — did they close out?
- Response gaps — messages where the user was mentioned/asked a direct question and either didn't respond or responded much later
- Moments where communication was unclear, incomplete, or came after someone else had to follow up
- Tasks the user said they'd do but aren't reflected in subsequent messages as done
- Signs of overextension — too many parallel tracks, rate limits hit, things dropped
- Any friction, misalignment, or correction from a teammate

### 3b. Meeting notes
List and read files in `Data\Meetings\` modified on or after `window_start`. Look for:
- Action items assigned to the user — were they completed, or carried forward without closure?
- Meetings where the user arrived without full context on a topic they were expected to own
- Moments where a stronger earlier signal could have prevented a later problem
- Patterns of reactive rather than proactive behavior
- Any instance of scope or timeline being promised that later required walking back
- Communication that should have happened before the meeting but didn't

### 3c. Claude sessions
List and read files in `Data\Claude\` modified on or after `window_start`. Look for:
- Sessions that indicate rework (fixing something that was just built)
- Evidence of scope creep or rabbit-holing beyond what was actually needed
- Commitments made in sessions that weren't followed through in other sources
