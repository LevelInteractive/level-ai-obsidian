...
Read ALL of the following at once (parallel tool calls in a single response):
...
3. Any meeting files in `Data/Meetings/` whose filename contains the client/person name
   (case-insensitive). If there are more than 6, prioritize the most recent 6 by date.
4. The most recent Slack Activity file in `Data/Work/Slack Activity/` — search for mentions
   of the client/person name

Reading widely is more valuable than reading deeply. Better to skim 8 sources than read 2
exhaustively — breadth reveals patterns that single sources hide.
...
**Confidence** (use CLAUDE.md definitions):
