...
- **Graph state**: `.claude\similarity-graph.json` — shared with `ob-similarity-update`; that skill
...
- **Synthesis spec**: `.claude\skills\ob-similarity-consolidate\references\synthesis.md`
- **Tag vocabulary**: `.config\tagging.md` (shared vault config, not a dependency on any other skill)

## Step 0 — Load the graph

Read `.claude\similarity-graph.json`. If it doesn't exist or `flagged` is empty and no cluster is
...
2. **Apply `references/synthesis.md`**: page templates, the three-way confidence model, the merge
   ...
   Claude-session handling rules for any `Data\Claude\` member.
...
4. Write the updated graph back to `.claude\similarity-graph.json` — `flagged` should now be empty
...
- **Core synthesis non-negotiables** (detailed in `references/synthesis.md`): never modify
  ...
  with Claude session files, prefer incremental edits over full page rewrites.
